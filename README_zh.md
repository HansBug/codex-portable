...
- 当前文档固定版本：[`v0.142.4`](https://github.com/HansBug/codex-portable/releases/tag/v0.142.4)，发布日期 `2026-06-29`
...
本 README 固定到当前已发布的 `v0.142.4`。下表会在 release 附件全部上传完成后，基于真实附件列表自动刷新。
...
下面的命令会直接安装当前文档固定版本 `v0.142.4`。如果你需要更老的 Codex 版本，请打开 Releases 页面，把下面固定写死的 tag 和资源文件名替换成目标版本即可。
...
- 当前版本：`https://github.com/HansBug/codex-portable/releases/tag/v0.142.4`
- 全部版本：`https://github.com/HansBug/codex-portable/releases`

### 2. 下载正确的归档文件

Linux 和 macOS 推荐优先下载 `tar.gz`，因为它能保留可执行权限。当前 `v0.142.4` 的真实附件已经在上面的附件表中全部列出。
...
1. 解压 `codex-portable-x86_64-pc-windows-msvc-v0.142.4.zip`
...
当前文档固定版本 `v0.142.4` 的 Linux `Fast install`、`Manual / offline install` 和 `Basic config` 已在干净的 Ubuntu Docker 容器中实际验证后再发布。
